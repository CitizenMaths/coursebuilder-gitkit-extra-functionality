...
_CM_NAMESPACE = '<YOUR_COURSE_NAMESPACE_HERE>'
_COURSE_COMPLETION_PROPERTY_NAME = 'linear-course-completion'
_SKILL_COMPLETION_PROPERTY_NAME = 'skill-completion'
...
class EmailUpdatePolicy(users.EmailUpdatePolicy):
    ...
    @classmethod
    def is_gitkit_user_registered(cls, user_id):
        with utils.Namespace(_CM_NAMESPACE):
            ts = models.Student.get_by_user_id(user_id)
            if ts and ts.is_enrolled:
                return True
            else:
                return False

    @classmethod
    def find_gae_user(cls, email):
        with utils.Namespace(_CM_NAMESPACE):
            if models.Student.is_email_in_use(email):
                val = models.Student.get_first_by_email(email)
    ...
    @classmethod
    def copy_gae_user(cls, new_user_id, email, gae_student):
        with utils.Namespace(_CM_NAMESPACE):
            student = models.Student._add_new(new_user_id, email)
            _LOG.info('new user created to copy gae user for user id:')
            _LOG.info(student.user_id)
            student.additional_fields = gae_student.additional_fields
            student.is_enrolled = True
            student.enrolled_on = gae_student.enrolled_on
            student.last_seen_on = gae_student.last_seen_on
            student.scores = gae_student.scores
            student.labels = gae_student.labels
            student.name = gae_student.name
            _LOG.info('finished copy')
            _LOG.info(student.additional_fields)
            student.put()
        return student

    @classmethod
    def copy_gae_user_completion_info(cls, gae_student, gitkit_student):
        with utils.Namespace(_CM_NAMESPACE):
            v = models.StudentPropertyEntity.get(gae_student, _COURSE_COMPLETION_PROPERTY_NAME)
            if v is not None:
                _LOG.info('value for course completion:')
                _LOG.info(v.name)
                _LOG.info(v.value)
                _LOG.info(v.updated_on)
                _LOG.info(v.key().name())
                copy = models.StudentPropertyEntity.create(gitkit_student, _COURSE_COMPLETION_PROPERTY_NAME)
                copy.value = v.value
                copy.updated_on = v.updated_on
                _LOG.info('copied progress data')
                _LOG.info(copy.name)
                _LOG.info(copy.value)
                _LOG.info(copy.key().name())
                copy.put()

    @classmethod
    def copy_gae_user_answers(cls, gae_user_id, gitkit_user_id):
        with utils.Namespace(_CM_NAMESPACE):
            sa = models.StudentAnswersEntity.get_by_key_name(gae_user_id)
            if sa is not None:
                _LOG.info(sa)
                _LOG.info(sa.data)
                _LOG.info(sa.updated_on)
                answers = models.StudentAnswersEntity(key_name=gitkit_user_id)
                answers.data = sa.data
                answers.updated_on = sa.updated_on
                _LOG.info('finished copying student answers')
                _LOG.info(answers.key().name())
                _LOG.info(answers.data)
                answers.put()
            else:
                _LOG.info('No student answers found!')
...
class GitkitService(object):
    ...
    def __init__(self,
        ...
        self._instance = gitkitclient.GitkitClient(
            ...
            server_api_key=server_api_key, widget_url=widget_url)#identity toolkit v0.1.6
            #project_id='citizenmaths-phase-2', widget_url=widget_url)#identity toolkit v0.1.11
...
class RequestContext(webapp2.RequestContext):
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        _LOG.info(Runtime.get_current_user())
        _LOG.info('setting current user to none')
        Runtime.set_current_user(None)
        _LOG.info(Runtime.get_current_user())
...
class Runtime(object):
    ...
    def get_current_user(cls):
        try:
            return cls._threadlocal.__getattribute__('current_user')
        except:
            return None

    @classmethod
    def set_current_user(cls, user):
        cls._threadlocal.__setattr__('current_user', user)
...
class SignInHandler(BaseHandler):
    ...
    def get(self):
        ...
        _LOG.info('start')
        _LOG.info(runtime_config.client_id)
        _LOG.info('end')
